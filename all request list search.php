...
<body>
	...
	<div class="banner-bottom">
	   ...
	   <input type="submit" value="serach"/>
				...
				<h3 class="tittle three"> </h3>
				
				
				<?php
$connection = mysqli_connect('localhost','root','','blood_donor_organization');
$output='';
...
if(isset($_POST['search'])){
    ...
    $query = mysqli_query($connection,"SELECT * FROM request WHERE r_name LIKE '%$searchkey%' OR r_location LIKE '%$searchkey%'") or die("Could not search!");
    ...
    if($count == 0){
        ...
        while($row=mysqli_fetch_array($query)){
			...
			$user_id=$row['user_Id'];
			$r_blood_amount=$row['r_blood_amount'];

            $output .='<div>'.$r_name.'<div>' .$r_location.' '.$r_blood_amount.'</div>';
